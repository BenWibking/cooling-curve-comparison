...
//#error "COOL_LOW_TEMPERATURES is enabled!"
#endif
...
// copied from system.c to avoid having to include all of system.c
...
double DoCooling(double u_old, double rho, double dt, double ne_guess, struct species_data *data)
{
    ...
    du = dt * data->DtInternalEnergy / ( (UNIT_SPECEGY_IN_CGS/UNIT_TIME_IN_CGS) * (PROTONMASS/HYDROGEN_MASSFRAC));
    u_old += 0.5*du;
    u = CallGrackle(u_old, rho, dt, ne_guess, &grackle_species, 0);
    ...
    LambdaNet = CoolingRateFromU(u, rho, ne_guess, data);

    /* bracketing */
    if(u - u_old - ratefact * LambdaNet * dt < 0)	/* heating */
    {
        u_upper *= sqrt(1.1); u_lower /= sqrt(1.1);
        while((iter_upper<MAXITER)&&(u_upper - u_old - ratefact * CoolingRateFromU(u_upper, rho, ne_guess, data) * dt < 0))
        {
    ...
    if(u - u_old - ratefact * LambdaNet * dt > 0) /* cooling */
    {
        ...
        while((iter_lower<MAXITER)&&(u_lower - u_old - ratefact * CoolingRateFromU(u_lower, rho, ne_guess, data) * dt > 0))
        {
    ...
    /* crash condition */
    if(iter >= MAXITER) {printf("failed to converge in DoCooling(): u_in=%g rho_in=%g dt=%g ne_in=%g\n",u_old,rho,dt,ne_guess); endrun(10);}
    ...
    double u_in=specific_energy_codeunits_toreturn, rho_in=data->Density*All.cf_a3inv, mu=1, temp, ne=1, nHI=data->HI, nHII=data->HII, nHeI=1, nHeII=0, nHeIII=0;
    temp = ThermalProperties(u_in, rho_in, data, &mu, &ne, &nHI, &nHII, &nHeI, &nHeII, &nHeIII);
    data->HI = nHI; data->HII = nHII;
#ifdef RT_CHEM_PHOTOION_HE
    data->HeI = nHeI; data->HeII = nHeII; data->HeIII = nHeIII;
#endif
#endif
...
double GetCoolingTime(double u_old, double rho, double ne_guess, struct species_data *data)
{
    ...
    double LambdaNet = CallGrackle(u_old, rho, 0.0, ne_guess, &grackle_species, 1);
    ...
    u_old *= UNIT_SPECEGY_IN_CGS;
    double nHcgs = HYDROGEN_MASSFRAC * rho / PROTONMASS;	/* hydrogen number dens in cgs units */
    double LambdaNet = CoolingRateFromU(u_old, rho, ne_guess, data);
...
double convert_u_to_temp(double u, double rho, struct species_data *data, double *ne_guess, double *nH0_guess, double *nHp_guess, double *nHe0_guess, double *nHep_guess, double *nHepp_guess, double *mu_guess)
{
    ...
    temp_guess = (EOS_GAMMA-1) / BOLTZMANN * u * PROTONMASS;
    *mu_guess = Get_Gas_Mean_Molecular_Weight_mu(temp_guess, rho, nH0_guess, ne_guess, 0., data);
    temp = (EOS_GAMMA-1) / BOLTZMANN * u * PROTONMASS * (*mu_guess);
    
    do
    {
        ne_old = *ne_guess;
        find_abundances_and_rates(log10(temp), rho, data, -1, 0, ne_guess, nH0_guess, nHp_guess, nHe0_guess, nHep_guess, nHepp_guess, mu_guess);
        temp_old = temp;
        temp_new = (EOS_GAMMA-1) / BOLTZMANN * u * PROTONMASS * (*mu_guess);
        ...
        if(fabs(temp-temp_old_old)/(temp+temp_old_old) < 1.e-4) {double wt=get_random_number(12*iter+340*ThisTask); temp=(wt*temp_old + (1.-wt)*temp_new);}
        temp_old_old = temp_old;
        iter++;
        if(iter > (MAXITER - 10)) {printf("-> temp=%g/%g/%g ne=%g/%g mu=%g rho=%g max=%g iter=%d\n", temp,temp_new,temp_old,*ne_guess,ne_old, (*mu_guess) ,rho,max,iter);}
    }
    while(
    ...
    if(iter >= MAXITER) {printf("failed to converge in convert_u_to_temp(): u_input= %g rho_input=%g n_elec_input=%g\n", u_input, rho_input, *ne_guess); endrun(12);}
                                 ...
                                 double *mu_guess)
{
    ...
    {
        nH0 = 1.0; nHe0 = yhelium(data); nHp = 0; nHep = 0; nHepp = 0; n_elec = 0;
        *nH0_guess=nH0; *nHe0_guess=nHe0; *nHp_guess=nHp; *nHep_guess=nHep; *nHepp_guess=nHepp; *ne_guess=n_elec;
        *mu_guess=Get_Gas_Mean_Molecular_Weight_mu(pow(10.,logT), rho, nH0_guess, ne_guess, 0, data);
        return 0;
    }
    if(logT >= Tmax)		/* everything is ionized */
    {
        nH0 = 0; nHe0 = 0; nHp = 1.0; nHep = 0; nHepp = yhelium(data); n_elec = nHp + 2.0 * nHepp;
        *nH0_guess=nH0; *nHe0_guess=nHe0; *nHp_guess=nHp; *nHep_guess=nHep; *nHepp_guess=nHepp; *ne_guess=n_elec;
        *mu_guess=Get_Gas_Mean_Molecular_Weight_mu(pow(10.,logT), rho, nH0_guess, ne_guess, 1.e3, data);
        return 0;
    }
    ...
    dt = All.Timebase_interval / All.cf_hubble_a; // dtime [code units]
    ...
    double c_light_ne=0, Sigma_particle=0, abs_per_kappa_dt=0;
        double L_particle = Get_Particle_Size(data)*All.cf_atime; // particle effective size/slab thickness
        double cx_to_kappa = HYDROGEN_MASSFRAC / PROTONMASS * UNIT_MASS_IN_CGS; // pre-factor for converting cross sections into opacities
        Sigma_particle = cx_to_kappa * data->Mass / (M_PI*L_particle*L_particle); // effective surface density through particle
        abs_per_kappa_dt = cx_to_kappa * C_LIGHT_CODE_REDUCED * (data->Density*All.cf_a3inv) * dt; // fractional absorption over timestep
        nH0 = data->HI; // need to initialize a value for the iteration below
#ifdef RT_CHEM_PHOTOION_HE
        nHe0 = data->HeI; nHep = data->HeII; // need to intialize a value for the iteration below
#endif
#endif
    ...
    do
    {
        ...
        /* add in photons from explicit radiative transfer (on top of assumed background) */
        {
            ...
            for(k = 0; k < N_RT_FREQ_BINS; k++)
            {
                ...
                {
                    double c_ne_time_n_photons_vol = c_light_ne * rt_return_photon_number_density(data,k); // gives photon flux
...
#ifdef RT_CHEM_PHOTOION
        {nH0 = (data->HI + fac_noneq_cgs * aHp) / (1 + fac_noneq_cgs * (aHp + geH0 + gJH0ne));} // slightly more general formulation that gives linear update but interpolates to equilibrium solution when dt >> dt_recombination
#endif
        nHp = 1.0 - nH0;		/* eqn (34) */
        ...
        {
            nHep = yhelium(data) / (1.0 + (aHep + ad) / (geHe0 + gJHe0ne) + (geHep + gJHepne) / aHepp);	/* eqn (35) */
        ...
        {
            double yHe = yhelium(data); // will use helium fraction below
            nHep = data->HeII + yHe * fac_noneq_cgs * (geHe0 + gJHe0ne) - data->HeIII * (fac_noneq_cgs*(geHe0 + gJHe0ne - aHepp) / (1.0 + fac_noneq_cgs*aHepp));
            ...
            nHepp = (data->HeIII + data->HeII * fac_noneq_cgs*(geHep + gJHepne)) / (1. + fac_noneq_cgs*aHepp);
...
#endif
        if(!isfinite(n_elec)) {printf("niter=%d logT=%g n_elec/old=%g/%g nHp/nHep/nHepp=%g/%g/%g nHcgs=%g yHe=%g dt=%g shieldfac/local_gammamult=%g/%g aHp/aHep/aHepp=%g/%g/%g geH0/geHe0/geHep=%g/%g/%g gJH0ne/gJHe0ne/gJHepne=%g/%g/%g \n",niter,logT,n_elec,neold,nHp,nHep,nHepp,nHcgs,yhelium(data),dt,shieldfac,local_gammamultiplier,aHp,aHep,aHepp,geH0,geHe0,geHep,gJH0ne,gJHe0ne,gJHepne);}
        ...
        if(niter > (MAXITER - 10)) {printf("n_elec= %g/%g/%g yh=%g nHcgs=%g niter=%d\n", n_elec,neold,nenew, yhelium(data), nHcgs, niter);}
    }
    while(niter < MAXITER);
    
    if(niter >= MAXITER) {printf("failed to converge in find_abundances_and_rates(): logT_input=%g  rho_input=%g  ne_input=%g shieldfac=%g cooling_return=%d", logT_input, rho_input, ne_input, shieldfac, return_cooling_mode); endrun(13);}
    ...
    *mu_guess=Get_Gas_Mean_Molecular_Weight_mu(pow(10.,logT), rho, nH0_guess, ne_guess, sqrt(shieldfac)*(gJH0/2.29e-10), data);
    /* if this is a cell, update some of its thermodynamic stored quantities */
    {
        data->Ne = n_elec;
#ifdef OUTPUT_MOLECULAR_FRACTION
        data->MolecularMassFraction = Get_Gas_Molecular_Mass_Fraction(data, pow(10.,logT), DMAX(nH0,0), sqrt(shieldfac)*(gJH0/2.29e-10) , 1);
#endif
    }
...
double CoolingRateFromU(double u, double rho, double ne_guess, struct species_data *data)
{
    ...
    double temp = convert_u_to_temp(u, rho, data, &ne_guess, &nH0_guess, &nHp_guess, &nHe0_guess, &nHep_guess, &nHepp_guess, &mu);
    return CoolingRate(log10(temp), rho, ne_guess, data);
}
 ...
 */
double CoolingRate(double logT, double rho, double n_elec_guess, struct species_data *data)
{
    ...
    double LambdaPElec = 0.; // for debugging
    double eps_pe = NAN;
    ...
    Tdust = get_equilibrium_dust_temperature_estimate(data);
#endif
#endif
    ...
    {
        double L_particle = Get_Particle_Size(data)*All.cf_atime; // particle effective size/slab thickness
        double dt = All.Timebase_interval / All.cf_hubble_a; // dtime [code units]
        Sigma_particle = data->Mass / (M_PI*L_particle*L_particle); // effective surface density through particle
        abs_per_kappa_dt = C_LIGHT_CODE_REDUCED * (data->Density*All.cf_a3inv) * dt; // fractional absorption over timestep
    ...
    if(logT < Tmax)
    {
        ...
        Lambda = find_abundances_and_rates(logT, rho, data, shieldfac, 1, &n_elec, &nH0, &nHp, &nHe0, &nHep, &nHepp, &mu);
        ...
        if((J_UV != 0)&&(logT > Tmin+0.5*deltaT)&&(logT > 4.00))
        {
            ...
            {data->MetalCoolingRate = LambdaMetal;}
#endif
        }
#endif
        ...
        if((logT <= 5.3) && (logT > Tmin+0.5*deltaT))
        {
            ...
            /* now add the dust cooling/heating terms */ // NOTE(bdw): this is *gas-grain* collisional heating, NOT photoelectric heating!
            LambdaDust = 1.116e-32 * (Tdust-T) * sqrt(T)*(1.-0.8*exp(-75./T)) * Z_sol;  // Meijerink & Spaans 2005; Hollenbach & McKee 1979,1989 //
#ifdef RT_INFRARED
            {LambdaDust = get_rt_ir_lambdadust_effective(T, rho, &nH0, &n_elec, data);} // call our specialized subroutine, because radiation and gas energy fields are co-evolving and tightly-coupled here //
#endif
        ...
        if(nHcgs > MIN_REAL_NUMBER)
        {
            ...
            for(k = 0; k < N_RT_FREQ_BINS; k++)
            {
                if((k==RT_FREQ_BIN_H0)||(k==RT_FREQ_BIN_He0)||(k==RT_FREQ_BIN_He1)||(k==RT_FREQ_BIN_He2))
                {
                    double c_nH_time_n_photons_vol = c_light_nH * rt_return_photon_number_density(data,k); // gives photon flux
        ...
        double u_gamma_xr=0, Tc_xr=0, prefac_for_compton = 1.35e-23 * (data->Density*All.cf_a3inv/data->Mass) * UNIT_PRESSURE_IN_CGS / nHcgs; // convert Rad_E_gamma to (u_gamma*c)*sigma_Thompson*(4*kB)/(me*c^2) in CGS (last 1/n is to make it in appropriate "Lambda" units)
#if defined(RT_HARD_XRAY)
        u_gamma_xr=data->Rad_E_gamma[RT_FREQ_BIN_HARD_XRAY]; Tc_xr=1.7e7; if(T<Tc_xr) {Heat += prefac_for_compton*u_gamma_xr*(Tc_xr-T);}
#endif
#if defined(RT_SOFT_XRAY)
        u_gamma_xr=data->Rad_E_gamma[RT_FREQ_BIN_SOFT_XRAY]; Tc_xr=3.6e6; if(T<Tc_xr) {Heat += prefac_for_compton*u_gamma_xr*(Tc_xr-T);}
#endif
#endif
...
//#if defined(FLAG_NOT_IN_PUBLIC_CODE) || defined(RT_PHOTOELECTRIC)
        ...
        if((T < 1.0e6))
        {
            ...
            double photoelec = data->Rad_E_gamma[RT_FREQ_BIN_PHOTOELECTRIC] * (data->Density*All.cf_a3inv/data->Mass) * UNIT_PRESSURE_IN_CGS / 3.9e-14; // convert to Habing field //
            if(photoelec > 0) {if(photoelec > 1.e4) {photoelec = 1.e4;}}
#endif
            const double photoelec = 1.7; // G0 = 1.7 [Draine value for solar neighborhood]
            if(photoelec > 0)
            {
                LambdaPElec = 1.3e-24 * photoelec / nHcgs * (data->Metallicity[0] / All.SolarAbundances[0]);
                double x_photoelec = photoelec * sqrt(T) / (0.5 * (1.0e-12+n_elec) * nHcgs);
                eps_pe = 0.049/(1+pow(x_photoelec/1925.,0.73)) + 0.037*pow(T/1.0e4,0.7)/(1+x_photoelec/5000.);
                LambdaPElec *= eps_pe;
                Heat += LambdaPElec;
            }
        }
//#endif
...
#if defined(OUTPUT_COOLRATE_DETAIL)
    {
        data->CoolingRate = Lambda;
        data->HeatingRate = Heat;
        data->PhotoheatingRate = LambdaPElec;
        data->PhotoheatingEfficiency = eps_pe;
    }
#endif
     ...
     */
    if( (nHcgs > 0.1) )  /* don't bother at very low densities, since youre not optically thick, and protect from target=-1 with GALSF_EFFECTIVE_EQS */
    {
        double surface_density = evaluate_NH_from_GradRho(data->Gradients.Density,data->Hsml,data->Density,data->NumNgb,1,data);
        ...
        if(T < 1500.)
        {
            ...
            kappa_eff *= data->Metallicity[0]/All.SolarAbundances[0];
            ...
            double k_molecular = 0.1 * data->Metallicity[0]; /* molecular line opacities */
            double k_Hminus = 1.1e-25 * sqrt(data->Metallicity[0] * rho) * pow(T,7.7); /* negative H- ion opacity */
            double k_Kramers = 4.0e25 * (1.+HYDROGEN_MASSFRAC) * (data->Metallicity[0]+0.001) * rho / (T*T*T*sqrt(T)); /* free-free, bound-free, bound-bound transitions */
...
#if defined(OUTPUT_COOLRATE_DETAIL)
    {data->NetHeatingRateQ = Q;}
#endif
#ifdef OUTPUT_MOLECULAR_FRACTION
    {data->MolecularMassFraction = Get_Gas_Molecular_Mass_Fraction(data, T, DMAX(nH0,0), sqrt(shieldfac)*(gJH0/2.29e-10) , 1);;}
#endif
    ...
    {Q += data->DtInternalEnergy / nHcgs;}
#if defined(OUTPUT_COOLRATE_DETAIL)
    {data->HydroHeatingRate = data->DtInternalEnergy / nHcgs;}
#endif
#endif
...
void ReadMultiSpeciesTables(int iT)
{
    ...
    //printf(" ..opening Cooling Table %s \n",fname);
...
void ReadIonizeParams(char *fname)
{
    ...
    //printf(" ..read ionization table [TREECOOL] with %d entries in file `%s'. Make sure to cite the authors from which the UV background was compiled! (See user guide for the correct references).\n", nheattab, fname);
}
...
void InitCool(void)
{
    ...
    All.cf_atime = All.cf_a2inv = All.cf_a3inv = All.cf_afac1 = All.cf_afac2 = All.cf_afac3 = All.cf_hubble_a = All.cf_hubble_a2 = 1;
...
double ThermalProperties(double u, double rho, struct species_data *data, double *mu_guess, double *ne_guess, double *nH0_guess, double *nHp_guess, double *nHe0_guess, double *nHep_guess, double *nHepp_guess)
{
    ...
    double temp = convert_u_to_temp(u, rho, data, ne_guess, nH0_guess, nHp_guess, nHe0_guess, nHep_guess, nHepp_guess, mu_guess);
    return temp;
}
